...
def calcula_saldo(lancamentos) -> float:
    trat = list(map(lambda x: x[0] if x[1] == 'C' else -1*x[0],lancamentos))
    resp = reduce(lambda x,y: x+y,trat)
    return(resp)

lancamentos1 = [(200,'D'),(300,'C'),(100,'C')]
print(calcula_saldo(lancamentos1))